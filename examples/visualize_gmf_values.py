...
def analyse_and_plot(axes, ipp_num):
    gmf_vars, gmf_tx = analyze_gmf.integrate_and_match_ipps(
        (reader, chnl),
        (reader, chnl),
        start_sample + ipp_num*delta_samples,
        params,
    )
    ...
    axes[0].set_title(f"Integration number {ipp_num}")
...
if args.ipp < 0:
    ani = FuncAnimation(fig, lambda ind: analyse_and_plot(axes, ind), frames=range(ipp_anim))
else:
