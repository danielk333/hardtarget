...
Analysis accepts an optional parameter job, restricting processing to that
particular job. This corresponds to processing a
subset of the data, i.e. the war data processed by **radardef**.

Job objects are represented as a Job type, which is a NamedTuple.
...
```python
    ...
    analyse(*args, job=Job(idx=0, N=1), **kwargs)

    // alt 2 - subdivision,
    for index in range(N):
        analyse(*args, Job(idx=index, N=N), **kwargs)
```
...
```shell
    mpiexec -n 4 hardtarget -v analyse /data/path --config cfg.ini --progress -o /data/mf
```

Here is the relevant code within the `hardtarget analyse` script.
