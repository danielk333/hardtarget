...
for signal_time in [0.02, 0.2]:
    ...
    axes_ex[0].plot(
        fvec, spec[:, daccs0_ind], c="k", label=f"true delta acceleration {daccs[daccs0_ind]} m/s^2"
    )
    axes_ex[0].plot(
        fvec, spec[:, mspec_a_ind], c="b", label=f"peak delta acceleration {daccs[mspec_a_ind]} m/s^2"
    )
