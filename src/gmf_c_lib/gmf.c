...
// complex arrays are indexed as z_tx[2*index] = real part, z_tx[2*index+1] = imaginary part
...
void compute_echo_signal(
    ...
    for (int ti = 0; ti < z_tx_len; ti++) {
        ...
        echo[tidx][0] +=
            z_tx[tx_real_i] * z_rx[(rx_window[ti] + rg) * 2] - z_tx[tx_imag_i] * z_rx[(rx_window[ti] + rg) * 2 + 1];
        // rea*imb + ima*reb
        // Imag part of z_t[ti]x*z_rx[rg+ti]
        echo[tidx][1] +=
            z_tx[tx_real_i] * z_rx[(rx_window[ti] + rg) * 2 + 1] + z_tx[tx_imag_i] * z_rx[(rx_window[ti] + rg) * 2];
    }
}
...
int dpt(
    ...
    for (int ri = 0; ri < n_rg; ri++) {
        for (int sri = 0; sri < sub_res_len; sri++) {
            ...
            compute_echo_signal(
                echo, echo_len, z_tx, z_tx_len, z_rx, z_rx_len, sri, sub_res_len, dec, rgs[ri], rx_window
            );
