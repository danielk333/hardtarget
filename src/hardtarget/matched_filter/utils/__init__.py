...
def load_c_lib() -> ctypes.CDLL:
    ...
    __libpath__ = pymodule_dir / ("mfclib" + suffix)

    if __libpath__.is_file():
        # Then we open the created shared lib file
        mfclib = ctypes.cdll.LoadLibrary(str(__libpath__))

        # https://docs.python.org/3/library/ctypes.html#ctypes._CFuncPtr.argtypes

        mfclib.fgmf.restype = ctypes.c_int
        mfclib.fgmf.argtypes = [
        ...
        mfclib.fdpt.restype = ctypes.c_int
        mfclib.fdpt.argtypes = [
            nptype(np.complex64, 1),  # 1
            ctypes.c_int,  #  2
            nptype(np.complex64, 1),  # 3
            ctypes.c_int,  # 4
        ...
        mfclib.xcorr_echo_search.restype = ctypes.c_int
        mfclib.xcorr_echo_search.argtypes = [
            nptype(np.complex64, 1),  # 1
            ctypes.c_int,  #  2
            nptype(np.complex64, 1),  # 3
            ctypes.c_int,  # 4
