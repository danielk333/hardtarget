...
def test_parallel_analysis(method, get_test_data):
    ...
    if comm.rank == 0:
        assert res is not None, "No results from parallel analysis"
        data_loader = RadarDef().load_data(path=data_path, exp_def=exp)
        ...
        assert n_files == len(res["files"]), (
            f"Mismatch in amount of file generated, expected: {n_files}, generated: {len(res['files'])})"
        )
        assert len(res["files"]) == len(set(res["files"])), (
